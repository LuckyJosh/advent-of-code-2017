...
def generators_2(seedstring):
    seeds = [int(line.split()[-1]) for line in seedstring.split("\n")]

    factors = [16807, 48271]
    divider = [4, 8]
    modulus = 2147483647
    def num_generator(seed, a, b, c):
        while True:
            seed = (a * seed) % b
            while seed % c != 0:
                seed = (a * seed) % b
    ...
    generator_1 = num_generator(seeds[0], factors[0], modulus, divider[0])
    generator_2 = num_generator(seeds[1], factors[1], modulus, divider[1])
