...
def firewall_2(depth_range):
    ...
    for range_ in depth_range:
        if range_ != -1:
            layers.append(pyramid_range(range_, cutoff=50000*max_depth))
        else:
            layers.append(np.full(50000*max_depth, range_))

    positions_in_time = np.column_stack(layers)
    delay = None
    for i in tqdm.tqdm(range(0, 49999*max_depth)):
        caught_indices = np.argwhere(np.diag(positions_in_time[i:i + max_depth,:]) == 0).flatten()
        #print(positions_in_time[i:i + max_depth, :])
        #print(np.diag(positions_in_time[i:i + max_depth, :]))
